//Selim Can Yazar Hw2 152120191023
#include <iostream>
using namespace std;
...
double calculateTemperatureMembership(int temperature) {
    if (temperature <= 0) {
        ...
        cout << "Hava ılık." << endl;
        return double(temperature) / 15; 
    } else if (temperature >= 15 && temperature <= 30) {
        cout << "Hava sıcak." << endl;
        return double(temperature - 15) / 15; 
    } else { // temperature > 30
...
double calculateHumidityMembership(int humidity) {
    if (humidity <= 30) {
        ...
        return double(humidity - 30) / 30;  
    } else if (humidity > 60 && humidity <= 90) {
        cout << "Hava çok nemli." << endl;
        return double(humidity - 60) / 30;  
    } else { // humidity > 90
...
void printFuzzyCategory(double temperatureMembership, double humidityMembership) {
    ...
    // Sıcaklık ve nem kümelerindeki minimum üyelik derecelerini bul
    ...
    string resultCategory = fuzzyCategories[tempCategory][humCategory];

    // Hangi üyelik derecesinin daha küçük olduğunu belirle
    string smallerMembership = (temperatureMembership < humidityMembership) ? "Sıcaklık" : "Nem";

    // Sonucu ve daha küçük üyelik derecesini yazdır
    cout << "Hava Durumu: " << resultCategory << ", üyelik derecesi = " << ((temperatureMembership < humidityMembership) ? temperatureMembership : humidityMembership) << endl;
}
...
int main() {
    ...
    // Bulanık kategoriye göre sonucu yazdır
